...
func init() {
	android.RegisterModuleType("hentai_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = hentaiExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = hentaiExpandVariables(ctx, dep_file)
	...
	cmd := hentaiExpandVariables(ctx, String(g.properties.Cmd))
